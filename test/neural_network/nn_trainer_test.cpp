...
namespace oaz::nn {
TEST(TrainingBatch, Instantiation) { TrainingBatch<Game>(16); }
...
TEST(TrainingBatch, ReadElementFromMemory) {
  ...
  batch.readElementFromMemory(0, &board[0][0][0], &value_label,
                              &policy_label[0]);
  ...
  for (int i = 0; i != dimensions[0]; ++i)
    for (int j = 0; j != dimensions[1]; ++j)
      for (int k = 0; k != dimensions[2]; ++k)
        ASSERT_EQ(board[i][j][k],
                  (batch.getBoardTensor()
                       .template tensor<float, Game::NBoardDimensions + 1>()(
                           0, i, j, k)));

  /* ASSERT_EQ(value, (batch.getValueTensor().template tensor<float, 1>()(0)));
   */
  ASSERT_EQ(value_label,
            (batch.getValueLabelTensor().template tensor<float, 1>()(0)));

  /* for(int i=0; i!=Game::getPolicySize(); ++i) */
  /* 	ASSERT_EQ(policy[i], (batch.getPolicyTensor().template tensor<float,
   * 2>()(0, i))); */

  for (int i = 0; i != Game::getPolicySize(); ++i)
    ASSERT_EQ(policy_label[i],
              (batch.getPolicyLabelTensor().template tensor<float, 2>()(0, i)));
}
...
TEST(NNTrainer, Instantiation) { Trainer trainer(16, 1); }
...
TEST(NNTrainer, addTrainingExample) {
  ...
  trainer.addTrainingExample(&board, &value_label, &policy_label);
}
...
TEST(NNTrainer, CheckResults) {
  ...
  for (size_t i = 0; i != n_epochs; ++i) {
    for (size_t j = 0; j != batch_size; ++j) {
      trainer.addTrainingExample(&boards[j], &value_labels[j],
                                 &policy_labels[j]);
    }
  }
  ...
  for (size_t i = 0; i != data_after_training.size(); ++i) {
    ...
    evaluator.requestEvaluation(&game, &values[i], &policies[i],
                                DummyNotifier());
...
TEST(NNTrainer, MultiThreaded) {
  ...
  for (size_t i = 0; i != N_THREADS; ++i)
    workers.push_back(
        std::thread(&addTrainingExamples, &trainer, N_ITERATIONS));

  for (size_t i = 0; i != N_THREADS; ++i) workers[i].join();
}
}  // namespace oaz::nn
