...
TEST (TensorFlowTest, MatrixMultiplication) {
	...
	TF_Tensor* tensor1 = TF_AllocateTensor(
		data_type,
		dimensions.data(),
		num_dims,
		len
	...
	TF_Tensor* tensor2 = TF_AllocateTensor(
		data_type,
		dimensions.data(),
		num_dims,
		len
	);
	boost::multi_array_ref<float, 2> matrix1(
		(float*) TF_TensorData(tensor1),
		boost::extents[10000][10000]
	);
	boost::multi_array_ref<float, 2> matrix2(
		(float*) TF_TensorData(tensor2),
		boost::extents[10000][10000]
	);
