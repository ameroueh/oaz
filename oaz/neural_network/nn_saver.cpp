...
template <class Game>
NNTrainer<Game>::NNTrainer(SharedModelPointer model, size_t batch_size,
                           size_t epoch_size)
    : m_batch_size(batch_size), m_epoch_size(epoch_size), m_model(model) {}
...
template <class Game>
void NNTrainer<Game>::addTrainingExample(typename Game::Board* board,
  ...
  if (!m_batches.empty()) {
    ...
    if (!current_batch->full()) {
      ...
      current_batch->readElementFromMemory(index, &(*board)[0][0][0],
                                           value_label, &(*policy_label)[0]);
      ...
      addTrainingExample(board, value_label, policy_label);
    }
  } else {
    addNewBatch();
    m_batches.unlock();
...
void NNTrainer<Game>::trainFromBatch(Batch* batch) {
  ...
  for (size_t i = 0; i != batch->getSize(); ++i)
    m_model->Run({{"input:0", batch->getBoardTensor()},
                  {"value_labels:0", batch->getValueLabelTensor()},
                  {"policy_labels:0", batch->getPolicyLabelTensor()}},
                 {"loss"}, {"train"}, &outputs);
  std::cout << "Training batch loss: " << *(outputs[0].flat<float>().data())
            << std::endl;
}
...
void NNTrainer<Game>::maybeTrain() {
  ...
  if (m_batches.size() >= getEpochSize()) {
    ...
    while (!m_batches.empty()) {
      if (!m_batches.front()->availableForTraining()) break;
      batches_for_training.push(std::move(m_batches.front()));
      m_batches.pop_front();
    }
    m_batches.unlock();
    while (!batches_for_training.empty()) {
      UniqueBatchPointer batch_for_training =
          std::move(batches_for_training.front());
